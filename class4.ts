...
// question no 3 
 ...
 let aaraylist : string[] = array.filter ((item: string) => {
...
const temp : number [] =[0, 10, 20, 30, 40]; 
const  ArrList:number[] = temp.map((item:number) => {
    return (item * 9/5) + 32
 }) 
...
//Given an array of names ["Alice", "Bob", "Charlie", "David", "Emily"], use the
  //forEach method to log each name with an exclamation mark at the end, e.g., "Alice!"
  let arrayName : string[] = ["Alice", "Bob", "Charlie", "David", "Emily"];
 let IncludedName: string[] =  arrayName.map((item: string) => {
